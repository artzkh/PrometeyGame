...
class Database:
    ...
    async def get_user_attachment(self, peer_id, room):
        sql = f"SELECT attachment, current_clothes, room_lvl, {room} FROM Users WHERE peer_id = $1"
    ...
    async def update_status(self, peer_id, status):
        sql = "UPDATE Users SET status=$1 WHERE peer_id=$2"
        return await self.execute(sql, status, peer_id, execute=True)
